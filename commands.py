...
def cmd_edit(maybe_note: list = None) -> Output:
    ...
    if len(maybe_note) == 0:
        ...
        os.remove(TEMP_PATH)
        return Message("revised", "Revisions saved.")
...
def cmd_import(paths: list) -> Output:
    ...
    print("  Do not do this unless these files were created specifically for qnot!")
    do_parse: bool = confirm("parse")
    if confirm("add"):
        for p in paths:
            cmd_add([file_to_str(p)], pure=do_parse)
        return Message("imported", "Notes imported.")
...
def cmd_export(notes: list):
    ...
    if not os.path.exists(EX_PATH):
        os.mkdir(EX_PATH)
        print(f"Export directory created at {EX_PATH}")

    print(f'\nExporting {len(notes)} notes to {os.getcwd()}/{EX_PATH}')
    print("Notes will also remain in qnot.")
    if confirm("export"):
        for note in notes:
            full_path: str = EX_PATH + "qnot_exp_" + encode_norm(note.get_datem_obj()) + ".txt"
            if os.path.exists(full_path):
                print('File already exists:'), os.getcwd() + full_path
            else:
                print("Exporting:"), os.getcwd() + full_path
                note_to_file(note, full_path)
                export_suc += 1
        if export_suc == len(notes):
            return Message("exported", "All selected notes exported.")
        elif export_suc == 0:
            return Message("exported", "Export failed, all files already exist.")
        else:
            return Message("exported", "Some notes not exported.")
...
def cmd_delete(notes: list, getconf=True) -> Output:
    ...
    def delete():
        ...
        return Message("deleted", "Note(s) deleted.")
    ...
    if getconf:
        ...
        if confirm("delete"):
            return delete()
        return Message("cancel", "Nothing deleted.")
    else:
        return delete()
...
def cmd_search(query: list) -> Output:
    ...
    if result is None or len(result) == 0:
        return Message("nomatch", f"Match not found for `{' '.join(query)}`")
...
def cmd_merge(notes: list) -> Output:
    ...
    if total_notes == 0:
        return Message("cancel", "No notes provided to merge.")
    ...
    print("\nMerge tags, too? Choose 'Y' to include tags in merger, 'n' to include NO tags.")
    do_merge_tags: bool = confirm("merge tags")
    print("PERMANENTLY delete originals? Choose 'Y' to DELETE, 'n' to KEEP.")
    do_del_orig: bool = confirm("delete originals")
    print("Ready to merge?")
    if not confirm("merge with chosen options"):
        return Message("cancel", "Merge cancelled.")
    ...
    return Message("merged", "Notes merged.")
